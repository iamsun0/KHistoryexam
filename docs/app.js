...
(function () {
  ...
  // ---------- 오답 유사문제 ----------
  // 개념 태그의 희소도(IDF): 흔한 태그는 변별력이 낮아 가중치를 낮춘다. (1회 계산 후 캐시)
  let _idfCache = null;
  function tagIDF() {
    if (_idfCache) return _idfCache;
    const df = {}, N = Math.max(BANK.length, 1);
    for (const q of BANK) for (const t of (q.concept_tags || [])) df[t] = (df[t] || 0) + 1;
    const idf = {};
    for (const t in df) idf[t] = Math.log(N / df[t]) + 1; // 최소 1 보장
    _idfCache = idf;
    return idf;
  }
  // 문항별 최근 학습 상태: 각 id의 '마지막' 시도 기준 mastered(맞음)/wrong(틀림)
  function attemptStateById() {
    const s = loadStore(), st = {};
    for (const a of s.attempts) st[a.id] = a.correct ? "mastered" : "wrong"; // attempts는 시간순 → 마지막이 최신
    return st;
  }
  // 주제(topic)별 취약도: 유사문항 선정 시 같은 대주제에 가점
  function topicWeakness() {
    const s = loadStore(), byId = {};
    for (const q of BANK) byId[q.id] = q.topic;
    const m = {};
    for (const a of s.attempts) {
      const tp = byId[a.id]; if (!tp) continue;
      if (!m[tp]) m[tp] = { tries: 0, wrong: 0 };
      m[tp].tries++; if (!a.correct) m[tp].wrong++;
    }
    return m;
  }

  /**
   * 취약 개념 태그 집합을 받아 학습 효과가 높은 유사문항을 선정한다.
   * 점수 = Σ(매칭 태그 IDF × 개념 취약가중치) × 다중매칭보너스 × 난이도 × 주제가점 × 학습상태 × 지터
   * - 이미 맞힌 문항은 강한 감점, 이전에 틀린 문항은 가점(간격 반복), 미출제 문항은 우선.
   * - 주제 다양성 상한을 두어 한 대주제에 편중되지 않게 분산.
   */
  function similarQuestions(tags, limit) {
    const tagSet = new Set(tags);
    if (!tagSet.size) return [];
    const idf = tagIDF();
    const cstats = conceptStats();       // 개념별 {tries, wrong}
    const seen = attemptStateById();     // id → mastered|wrong
    const tw = topicWeakness();          // topic → {tries, wrong}

    // 요청된 각 태그의 가중치 = IDF × (1 + 오답수×(0.5+오답률))
    const tagW = {};
    for (const t of tags) {
      const cs = cstats[t] || { tries: 0, wrong: 0 };
      const rate = cs.tries ? cs.wrong / cs.tries : 0;
      tagW[t] = (idf[t] || 1) * (1 + cs.wrong * (0.5 + rate));
    }
    ...
    for (const q of BANK) {
      ...
      let s = 0, matched = 0;
      for (const t of qtags) if (tagSet.has(t)) { s += tagW[t]; matched++; }
      if (matched === 0) continue;

      if (matched > 1) s *= 1 + 0.2 * (matched - 1);        // 개념 클러스터 일치 보너스
      if (q.difficulty === "상") s *= 1.08;                  // 고난도 우선(심화 대비)

      const twp = tw[q.topic];                               // 취약 주제면 가점
      if (twp && twp.wrong > 0) s *= 1 + 0.15 * (twp.wrong / twp.tries);

      const stt = seen[q.id];
      if (stt === "mastered") s *= 0.15;                     // 이미 맞힌 문항 → 거의 배제
      else if (stt === "wrong") s *= 1.3;                    // 틀린 문항 → 간격 반복 우선
      // 미출제 문항은 1.0(기본 우선)

      s *= 1 + Math.random() * 0.08;                         // 소폭 지터: 세션마다 구성 다양화
      scored.push({ q, s });
    }
    scored.sort((a, b) => b.s - a.s);

    // 주제 다양성: 한 topic이 전체의 절반을 넘지 않도록 1차 선별 후 부족분 채움
    const cap = Math.max(2, Math.ceil(limit / 2));
    const perTopic = {}, picked = [], rest = [];
    for (const x of scored) {
      const tp = x.q.topic || "_";
      if ((perTopic[tp] || 0) < cap && picked.length < limit) { perTopic[tp] = (perTopic[tp] || 0) + 1; picked.push(x.q); }
      else rest.push(x.q);
    }
    for (const q of rest) { if (picked.length >= limit) break; picked.push(q); }
